...
from javsp.web.exceptions import MovieNotFoundError, CrawlerError
...
def _fill_movie_from_doc(doc, movie: MovieInfo, is_fc2: bool):
    ...
    dvdid = doc.xpath(
        "string(//div[@class='text-secondary'][span[normalize-space()='品番:']]/span[@class='font-medium'])")
    #   规整dvdId中的特殊子串
    replacements = {
        "FC2-PPV-": "FC2-",
        "-UNCENSORED-LEAK": "",
    ...
    actress = [a.strip() for a in
               doc.xpath("//div[@class='text-secondary'][span[normalize-space()='女優:']]//a/text()")]
    genre = [g.strip() for g in
             doc.xpath("//div[@class='text-secondary'][span[normalize-space()='ジャンル:']]//a/text()")]
    # if '-U' in dvdid:
    ...
    if cached_files:
        ...
        for fp in cached_files:
            try:
                ...
                cached_dvdid = doc.xpath(
                    "string(//div[@class='text-secondary'][span[normalize-space()='品番:']]/span[@class='font-medium'])").strip()
